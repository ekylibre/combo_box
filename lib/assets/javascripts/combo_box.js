...
(function ($) {
    ...
    $.initializeComboBox = function () {
        ...
        if (element.prop("alreadyBound") !== true) {
            ...
            element.prop("valueField", $('#' + element.data('value-container')));
            if ($.isEmptyObject(element.prop("valueField")[0])) {
                alert('An input ' + element.id + ' with a "data-combo-box" attribute must contain a "data-value-container" attribute (#' + element.data('value-container') + ')');
            }
            element.maxSize = parseInt(element.data('max-size'), 10);
    ...
    $("a[data-permute-combo-box]").live("click", function () {
        var elements = $($(this).data("permute-combo-box")), i;
        for (i = 0; i < elements.length; i += 1) {
            if ($(elements[i]).data("combo-box") === undefined || $(elements[i]).data("combo-box") === null) {
